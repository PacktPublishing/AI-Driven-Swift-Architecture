//
//  ChatBotWithTrainedModelView.swift
...
struct ChatBotWithTrainedModelView: View {

    // MARK: - State Properties

    /// The adapted language model loaded from the custom adapter bundle.
    @State private var adaptedModel: SystemLanguageModel?
        ...
        if SystemLanguageModel.default.isAvailable {
            NavigationStack {
                ...
                .onChange(of: configuration.instructions) {
                    if let model = adaptedModel {
                        if configuration.instructions.isEmpty {
                            session = LanguageModelSession(model: model)
                        } else {
                            session = LanguageModelSession(model: model, instructions: configuration.instructions)
                        }
                    }
                }
                .task {
                    do {
                        if let assetURL = Bundle.main.url(forResource: "my_adapter", withExtension: "fmadapter") {
                            let adapter = try SystemLanguageModel.Adapter(fileURL: assetURL)
                            adaptedModel = SystemLanguageModel(adapter: adapter)
                            session = LanguageModelSession(model: adaptedModel!)
                        }
                    } catch {
                        messages.append(Message(content: "Adapter requires a physical device with Apple Intelligence support.", isUser: false))
                    }
                }
            }
        } else {
